  ...
  return (
    <div>
      ...
      <div className="flex gap-4 border-b border-gray-200 mb-6 overflow-x-auto">
        {['registrations', 'loans', 'customers', 'add-customer'].map(tab => (
          <button
            key={tab}
      ...
      {activeTab === 'registrations' && (
        <div className="space-y-4">
          {registrations.length === 0 && <p className="text-gray-500">No pending registrations.</p>}
            ...
            <div key={req._id} className="bg-white p-6 rounded shadow border">
              <div className="flex justify-between">
                <div>
                  ...
                  <p className="text-gray-600">Email: {req.email} | WhatsApp: {req.whatsappNumber}</p>
                  <p className="text-gray-600">Aadhaar: {req.aadhaar} | PAN: {req.pan}</p>
                  <a href={req.photoUrl} target="_blank" rel="noreferrer" className="text-blue-500 text-sm hover:underline">View Photo</a>
                </div>
                <div className="flex gap-2 items-start">
                  <button onClick={() => handleApproveRegistration(req._id)} className="bg-emerald-600 text-white px-3 py-1 rounded hover:bg-emerald-700">Approve</button>
                  <button onClick={() => handleRejectRegistration(req._id)} className="bg-red-600 text-white px-3 py-1 rounded hover:bg-red-700">Reject</button>
                </div>
              </div>
            </div>
          ))}
      ...
      {activeTab === 'loans' && (
        <div className="space-y-4">
          {loanRequests.length === 0 && <p className="text-gray-500">No pending loan requests.</p>}
          {loanRequests.map(req => (
            <div key={req._id} className="bg-white p-6 rounded shadow border">
              <div className="flex justify-between items-center">
                <div>
                  <h3 className="text-lg font-bold text-gray-800">
                    Customer: {req.customer?.name} ({req.customer?.customerId})
                  </h3>
                  <p className="text-gray-600">Requested: ₹{req.requestedAmount} for {req.requestedDuration}</p>
      ...
      {activeTab === 'customers' && !selectedCustomer && (
        <div className="space-y-4">
          ...
          {filteredCustomers.length === 0 && <p className="text-gray-500">No customers found.</p>}
      ...
      {selectedCustomer && (
        <div>
            ...
            <div className="flex justify-between">
              ...
              <div className="flex flex-col gap-2">
                {selectedCustomer.status === 'ACTIVE' ? (
                   <button onClick={() => handleToggleCustomerStatus(selectedCustomer._id, selectedCustomer.status)} className="text-red-600 border border-red-600 px-3 py-1 rounded hover:bg-red-50">Deactivate</button>
                ) : (
                   <button onClick={() => handleToggleCustomerStatus(selectedCustomer._id, selectedCustomer.status)} className="text-green-600 border border-green-600 px-3 py-1 rounded hover:bg-green-50">Activate</button>
                )}
              </div>
            </div>
          </div>
          ...
          {selectedCustomer.loansData?.length === 0 && <p className="text-gray-500">No loans found.</p>}
      ...
      {selectedLoanRequest && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg shadow-xl max-w-lg w-full p-6">
            <h3 className="text-xl font-bold mb-4">Configure Loan Schedule</h3>
            <p className="mb-4 text-gray-600">
              Requested: ₹{selectedLoanRequest.requestedAmount} for {selectedLoanRequest.requestedDuration}
            </p>
            ...
            <form onSubmit={handleApproveLoan} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700">Final Duration</label>
              ...
              <div className="mt-6 flex justify-end gap-3">
                <button type="button" onClick={() => setSelectedLoanRequest(null)} className="px-4 py-2 text-gray-600">Cancel</button>
                <button type="submit" className="bg-emerald-600 text-white px-4 py-2 rounded hover:bg-emerald-700">Generate & Approve</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {activeTab === 'add-customer' && (
        <div className="bg-white p-6 rounded shadow border max-w-4xl">
          ...
          <form onSubmit={handleAddCustomerSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
               <div>
                  <label className="block text-sm font-medium text-gray-700">Set Initial Password</label>
